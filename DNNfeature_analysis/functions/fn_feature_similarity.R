fn_feature_similarity <- function(df_sim, info, sim_method) {
  ...
  distinctiveness      <- numeric(nrow(df_sim))
  ...
  for (cat in unique(df_sim$category)) {
    ...
    # ----------------------------------------------------------------------------
    if (params$features_do_log == TRUE) {
      feature_matrix <- sign(feature_matrix) * log(abs(feature_matrix) + 1)
    }
    
    if (params$features_do_center == TRUE || params$features_do_scale == TRUE) {
      feature_matrix <- scale(feature_matrix, center = params$features_do_center, 
                              scale = params$features_do_scale)
    }
    ...
    # ----------------------------------------------------------------------------
    # Convert similarity to distinctiveness according to Murdock (1960)    
    # ----------------------------------------------------------------------------
    d <- numeric(n_images)
    
    for (i in 1:n_images) {
      d[i] <- sum(abs(similarity_matrix[i, ]), na.rm = TRUE)
    }
    
    # Normalize distinctiveness to sum to 100%
    total_distinctiveness <- sum(d)
    distinctiveness[cat_indices] <- (d / total_distinctiveness)
  ...
  # ============================================================================
  df_sim[[paste0(sim_method, "_similar")]] <- simple_result_values
  df_sim[[paste0(sim_method, "_distnct")]] <- distinctiveness
